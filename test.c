...
int main() {
	...
	while (access(f, F_OK)!=-1 && i--) {
		if (i < 0) {
			exit(1);
		}
		asprintf(&f, NAME, i);
	}
	if ((fd = open(f, FLAGS, 0600)) == -1) {
		exit(1);
	}
	char *s = SOURCE;
	dprintf(fd, s, 10, 10, 10, 10, 10, 34, 37, 34, 10, 34, 37, 37, 34, 10, 34, s, 34, 10, 10, (i == -1) ? 0 : i, 10, 10, 34, 37, 34);
	asprintf(&str, COMPILE, i, i);
	asprintf(&name, "./Sully_%i", i);
	system(str);
	system(name);
}